package com.wisnu.q2_jsonmanipulation
...
fun running(){
    ...
    val listType = object : TypeToken<List<JsonClass>>() {}.type
    val json = JsonArray().jsonProfile
    var dataProfile: List<JsonClass> = gson.fromJson(json, listType)
    ...
    for (user in dataProfile.indices) {
        when (input) {
            ...
            2 -> {
                ...
                if (articles != 0) {
                    nameUser = dataProfile[user].username
                    println("$nameUser have Articles = $articles")
